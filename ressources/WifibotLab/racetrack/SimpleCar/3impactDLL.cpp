...
CameramanLookAtPointKeepDistance_t CMKD;
...
void Init()
{
   ...
   Terrain=iBodyCreate("racetrack\\body_1.35.ply");
   TerrainMesh=iBodyMeshCreate("racetrack\\visualtrack.x",Terrain);
   Gras=iBodyMeshCreate("racetrack\\visualgras.x",Terrain);
   //create dynamic car-body body and mesh
   CarBody=iBodySGCreate("simplecar_res\\carbody_.spg",2.13f);
   CarBodyMesh=iBodyMeshCreate("default_res\\carbody.x",CarBody);
   //set initial car position
   
   iBodyLocationSet(CarBody,&D3DXVECTOR3(0.0f,1.0f,0.0f),TRUE);
   ...
   iBodyDampingSet(CarBody,0.0f,0.999f,0.0f,0.991f);
   //create car-body shadow mesh
   CarBodyMeshShadow=iMeshBodyShadowCasterCreate("default_res\\carbody.x",CarBody,500.0f);
   ...
   iBodyBodyFrictionSet(CarBodyTerrain,4.5f);
   ...
   RollingWiseFriction = 13.0f;
   //suspension strength
   SuspensionSpring = 40.0f;
   //suspension hardness
   ...
   BODY* collisionbodies[10];
   collisionbodies[0] = Terrain;
   collisionbodies[1] = NULL;
   
   CMKD.Create(Camera,1.0f,25.0f,10.0f,20.0f,0.9f,0.15f,collisionbodies);
   //enable all bodies
   iBodyEnable(Terrain);
   iBodyEnable(CarBody);
...
void Run()
{
   ...
   //iCameraLookAt(Camera,&cameratarget,0.1f);
      ...
      D3DXVECTOR3 actorlocation;
   iBodyLocationCM(CarBody,&actorlocation);
   ...
   {
      iFloatTendTo(&SteerAngle,35.0f,0.3f);
   }
   ...
   if (iKeyDown(DIK_UP))
   {
      iFloatTendTo(&Spin,-WheelSpin*1,0.4f);
   }
   ...
   //However, if a [Ctrl] key is pressed, force Spin and power variables back to zero and 1.5f
   //respectively, to simulate brakes. The higher the power value, the stronger the brakes
   if ((iKeyDown(DIK_LCONTROL)) || (iKeyDown(DIK_RCONTROL)))
   {
      Spin = 0.0f;
      power = 1.5f;
   }
